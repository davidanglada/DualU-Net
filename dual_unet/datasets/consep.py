...
sys.path.append('/home/usuaris/imatge/david.anglada/PhD/tesi/dualunet/')
...
def consep2coco(data_dir, fold, out_dir):
    ...
    for img_idx, img_name in enumerate(os.listdir(img_dir)):
        ...
        for inst_id in uq_inst_ids:
            ...
            # print(inst_mask_i.shape)
            # print(inst_mask_i)
            
            # get the coordinates of the pixels for the current instance
            ...
            # print(inst_mask_i.astype(np.uint8))
            # print(inst_mask_i.astype(np.uint8).shape)
            # print(inst_mask_i.astype(np.uint8).dtype)
            # print(np.unique(inst_mask_i.astype(np.uint8)))
...
# config_path = "/home/usuaris/imatge/david.anglada/PhD/tesi/dualunet/config.yaml"
# import yaml
...
# dataset = build_consep_dataset(config, split='test')
# prova = dataset[0]
...
#         print(f"Error loading annotation at index {i}: {e}")
# # import ipdb; ipdb.set_trace()
# print(len(dataset))
# print(dataset[0][0])
